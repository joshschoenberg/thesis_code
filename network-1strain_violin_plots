...
def simulate_one_run(params):
    ...
    peak_infected = max(I_history)
    return I_eq, outbreak, assortativity, peak_infected
...
def plot_violin(total_infected, assortativity_values, title):
    ...
    # Create violin plot
    sns.violinplot(x=group_labels, y=flattened_data, density_norm="area", cut=0, palette=custom_palette, inner=None)
...
def main():
    ...
    for connection_bias in connection_bias_values:
        ...
        peak_infected = [res[3] for res in results]
        peak_infected_values.append(peak_infected)
        avg_assortativity_values.append(assortativity_avg)
        # total_infected.append([res[0] for res in results])
        print(f"Bias = {assortativity_avg} -> Outbreak Prob: {fraction_outbreak:.3f}")
    ...
    # -------------------------
    plot_violin(peak_infected_values, avg_assortativity_values, 'Size of Outbreak by Assortativity Level')
