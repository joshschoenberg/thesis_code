...
def main():
    ...
    for sigma in sigmas:
        ...
        print(f"sigma = {sigma:.2f} -> Outbreak rate: {np.mean(n_var2_outbreaks):.3f}, equilibrium infected: {np.mean(eq_infected):.3f}, peak infected: {np.mean(peak_infected_var_1):.3f}")
    ...
    plt.figure(figsize=(8, 5))
    plt.plot(np.array(sigmas), peak_infected_1, marker='o', linestyle='-', linewidth=4, label=f"Assortativity low ({a_1})")
    plt.plot(np.array(sigmas), peak_infected_2, marker='o', linestyle='-', linewidth=4, label=f"Assortativity high ({a_2})")
