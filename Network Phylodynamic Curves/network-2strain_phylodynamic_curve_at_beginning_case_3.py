...
def simulate_one_run(params):
    ...
    var2_outbreaks = 0
    num_infections_var_1 = 0
    num_infections_var_2 = 0
                        ...
                        if random.random() < beta * dt: 
                            ...
                            if t > var2_outbreak_time_threshold:
                                if np.random.rand() < p_mutation:
                                    I2[contact] = 1
                                    num_infections_var_2 += 1
                                else:
                                    I1[contact] = 1
                                    num_infections_var_1 += 1
                            else: 
                                I1[contact] = 1
                                num_infections_var_1 += 1
                            S[contact] = 0
    ...
    total_infected_var_1 = num_infections_var_1
    total_infected_var_2 = num_infections_var_2
    total_infected = total_infected_var_1 + total_infected_var_2
    ...
    return times, I1_t, I2_t, peak_infected, outbreak, var2_outbreaks, total_infected_var_1, total_infected_var_2, total_infected
...
def main():
    ...
    # params["var2_outbreak_threshold"] = 4

    params["var2_outbreak_time_threshold"] = 0
    ...
    total_infected_var_1_1 = []
    total_infected_var_2_1 = []
    total_infected_1 = []
    ...
    for sigma in sigmas:
        ...
        total_infected_var_1_1.append(np.mean([res[6] for res in results]))
        total_infected_var_2_1.append(np.mean([res[7] for res in results]))
        total_infected_1.append(np.mean([res[8] for res in results]))
        print(f"sigma = {sigma:.2f} -> Outbreak rate: {np.mean(n_var2_outbreaks):.3f}, Total infected var 1: {np.mean([res[6] for res in results]):.3f}, Total infected var 2: {np.mean([res[7] for res in results]):.3f}, Total infected: {np.mean([res[8] for res in results]):.3f}")
    ...
    total_infected_var_1_2 = []
    total_infected_var_2_2 = []
    total_infected_2 = []
    # Loop over each sigma value.
    for sigma in sigmas:
        params_loc = deepcopy(params)
        ...
        total_infected_var_1_2.append(np.mean([res[6] for res in results]))
        total_infected_var_2_2.append(np.mean([res[7] for res in results]))
        total_infected_2.append(np.mean([res[8] for res in results]))
        print(f"sigma = {sigma:.2f} -> Outbreak rate: {np.mean(n_var2_outbreaks):.3f}, Total infected var 1: {np.mean([res[6] for res in results]):.3f}, Total infected var 2: {np.mean([res[7] for res in results]):.3f}, Total infected: {np.mean([res[8] for res in results]):.3f}")
    
    ## Plotting total infected with different variants
